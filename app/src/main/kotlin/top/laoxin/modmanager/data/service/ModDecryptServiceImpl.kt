...
class ModDecryptServiceImpl @Inject constructor(private val archiveService: ArchiveService) :
    ...
    private fun decodeImageFileName(cacheFilePath: String, archivePath: String): String {
        ...
        val cacheDir =
            if (cacheFilePath.contains(PathConstants.MODS_ICON_PATH)) PathConstants.MODS_ICON_PATH else PathConstants.MODS_IMAGE_PATH
     ...
     */
    override suspend fun refreshModDetail(mod: ModBean): Result<ModBean> =
    ...
    private suspend fun refreshArchiveModDetail(mod: ModBean): Result<ModBean> {
        ...
        try {
            ...
            val extractedIcon = if (!mod.icon.isNullOrEmpty()) {
                ...
                Log.d(
                    TAG,
                    "Extracting icon: ${decodeImageFileName(File(mod.icon).path, archivePath)}"
                )
                extractImageToCache(
            ...
            val extractedImages = mod.images?.mapNotNull { imagePath ->
                ...
                Log.d(
                    TAG,
                    "Extracting image: ${decodeImageFileName(File(imagePath).path, archivePath)}"
                )
            ...
            val description = when {
                !mod.readmePath.isNullOrEmpty() -> {
                    extractReadmeContent(archivePath, mod.readmePath, mod.password)
                        ?: mod.description
                }

                !mod.fileReadmePath.isNullOrEmpty() -> {
                    extractReadmeContent(archivePath, mod.fileReadmePath, mod.password)
                        ?: mod.description
                }
